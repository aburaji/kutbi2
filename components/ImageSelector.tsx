...
const MediaCover: React.FC<{ item: Image; onZoomRequest: (imageUrl: string) => void; }> = ({ item, onZoomRequest }) => {
    return (
        <button
            onClick={() => onZoomRequest(item.url)}
        ...
        >
            <img 
                ...
                onError={(e) => e.currentTarget.src = 'https://placehold.co/400x600/1e293b/ffffff?text=Image'}
            />
        </button>
    );
};
...
const ImageSelector: React.FC<SelectorProps> = ({ 
    ...
    return (
        <div className="p-6 bg-white rounded-2xl shadow-lg border border-slate-200">
            <div className="flex justify-between items-center mb-4 border-b pb-3 border-slate-200 flex-wrap gap-2">
                <h2 className="text-2xl font-bold text-slate-800">مكتبة الصور</h2>
            ...
            <div className="my-4 relative">
                <input type="search" placeholder="ابحث عن صورة..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-10 pr-4 py-2.5 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-400" />
                        ...
                        return (
                            <div key={item.id} className="group flex flex-col bg-slate-100 rounded-xl shadow-md overflow-hidden border border-slate-200">
                                <div className="w-full h-80 block overflow-hidden bg-slate-200 animate-pulse relative flex flex-col items-center justify-center">
                                    ...
                                    <p className="text-slate-500 font-semibold mt-4">جاري إضافة الصورة...</p>
                                </div>
