            ...
            <div className="flex justify-between items-center mb-4 border-b pb-3 border-slate-200 flex-wrap gap-2">
                ...
                <div className="flex items-center gap-3">
                    <button onClick={onOpenNotesListModal} className="flex items-center gap-2 bg-white text-slate-700 font-semibold py-2 px-4 rounded-lg hover:bg-slate-50 transition-colors duration-200 text-sm shadow-sm border border-slate-300">
                        {/* FIX: Changed numeric strokeWidth property to a string. */}
                            ...
                            <div className="p-5 flex flex-col flex-grow">
                                ...
                                <div className="mt-auto pt-4 border-t border-slate-200 space-y-3">
                                    <div className="grid grid-cols-2 gap-2">
                                        ...
                                        <button onClick={() => item.script ? onViewScript(item) : onGenerateScript(item)} disabled={isGeneratingScript} className={`col-span-2 flex items-center justify-center text-center border font-semibold py-2 px-3 rounded-lg text-sm transition-colors ${isGeneratingScript ? 'bg-slate-200 text-slate-500 cursor-wait' : 'border-purple-600 text-purple-600 hover:bg-purple-50'}`}> { isGeneratingScript ? (<><svg className="animate-spin -ml-1 mr-3 h-5 w-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg><span>جارٍ الإنشاء...</span></>) : (item.script ? 'عرض النص' : 'إنشاء نص') }</button>
                                    </div>
                                    <button onClick={() => onAnalyze(item)} disabled={isAnalyzing || !canAnalyze} className="w-full bg-blue-600 text-white font-semibold py-2.5 px-4 rounded-lg hover:bg-blue-700 disabled:bg-slate-400 disabled:cursor-not-allowed transition-colors shadow hover:shadow-lg flex justify-center items-center"> {isCurrentlyAnalyzing ? (<><svg className="animate-spin -ml-1 mr-3 h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg><span>جارٍ التحليل...</span></>) : (<><svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M5 3v4M3 5h4M6 17v4m-2-2h4m5-16l2.286 6.857L21 12l-5.714 2.143L13 21l-2.286-6.857L5 12l5.714-2.143L13 3z" /></svg><span className="mr-2">تحليل النص</span></>)}</button>
                                </div>
                            </div>
                        </div>
                    )
